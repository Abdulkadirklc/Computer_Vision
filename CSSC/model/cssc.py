...
class CSSC(nn.Module):
    def __init__(self, last_stride=1, pretrain=True, num_classes=None):
        super().__init__()
        resnet = getattr(torchvision.models, 'resnet50')(weights=ResNet50_Weights.IMAGENET1K_V1)   

        print("resnetV1 weights are uses")
        ...
        self.fusion_conv = nn.Conv2d(2 * feature_dim, feature_dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        x_fused = torch.cat([x_cs_branch1, x_sc_branch2], dim=1)
        x_fused = self.fusion_conv(x_fused)
        
        # (B, 2048, H/16, W/16) + (B, 2048, H/16, W/16)
        x_cssc = self.backbone_layer43(x_fused)
        # (B, 2048, H/16, W/16)
